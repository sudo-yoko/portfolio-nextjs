...
4. アプリミドル基盤
    ...
    * [境界呼び出し設計](#flight_departure-境界呼び出し設計)
6. テスト＆モック
    * [node:test](#nodetest)
    * [next:jest](#nextjest)
...
* バックエンド API の呼び出し方式を以下とする。
	* サーバーサイド発バックエンド呼び出しの場合、サーバーサイドレンダリングでバックエンド API を呼び出す。
 	* クライアントサイド発バックエンド呼び出しの場合、BFF を経由してバックエンド API を呼び出す。
...
## :flight_departure: 境界呼び出し設計

<作成中>

バックエンドファサードにおける境界呼び出しの設計についての説明です。

* 「境界呼び出し」とは、クライアントからバックエンド API を利用する際に、クライアント → BFF → バックエンド API のように、クライアント／サーバーの境界をまたいで行う呼び出しを指しています。
* 「バックエンド・ファサード」は、その複数レイヤーにまたがる呼び出し手順や実装の複雑さを隠蔽し、クライアントにはシンプルな呼び出し関数だけを提供する窓口です。

本アプリケーションでは、境界呼び出しとバックエンド・ファサードの実装を統一するための共通の型を提供します。
