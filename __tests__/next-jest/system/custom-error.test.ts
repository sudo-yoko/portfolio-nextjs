import { customError, isErrorOf } from '@/modules/(system)/error-handlers/custom-error';
...
test('test1', () => {
  const e = customError('ActionError');
  if (e instanceof Error) {
    if (isErrorOf(e, 'AuthError')) {
      print('AuthError');
    } else if (isErrorOf(e, 'ActionError')) {
      print('ActionError');
    } else {
      print('Error');
    }
