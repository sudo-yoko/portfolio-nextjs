...
インストール
```
npm install -D jest ts-node @types/jest
```
* .ts モジュールのテストを行う構成。Reactのテストライブラリはインストールしない。
* `ts-node` - Next/jest が設定ファイル(jest.config.ts)を読む際に必要
...
📝  Configuration file created at /workspaces/portfolio-nextjs/my-app-16/jest.config.ts
```
